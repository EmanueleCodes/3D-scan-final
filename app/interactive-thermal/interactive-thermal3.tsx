...
const buildColorUniformData = (colors?: HeatmapProps["colors"]) => {
    ...
    while (entries.length < MAX_COLORS) {
        entries.push(entries[entries.length - 1] || DEFAULT_PALETTE[entries.length % DEFAULT_PALETTE.length])
    }
    ...
    const rawCount =
        colors?.paletteCount ??
        entries.filter((value, index) => index < MAX_COLORS && value !== undefined).length
    const count = Math.min(MAX_COLORS, Math.max(1, rawCount || DEFAULT_PALETTE.length))
...
export default function HeatmapComponent({
    ...
    useEffect(() => {
        ...
        if (!gl.getShaderParameter(vShader, gl.COMPILE_STATUS)) {
            log("Vertex shader compilation failed", gl.getShaderInfoLog(vShader))
            return
        }
        ...
        if (!gl.getShaderParameter(fShader, gl.COMPILE_STATUS)) {
            log("Fragment shader compilation failed", gl.getShaderInfoLog(fShader))
            return
        }
        ...
        const render = () => {
            ...
            if (uniformLocs.u_angle) gl.uniform1f(uniformLocs.u_angle, params.angle)
            if (uniformLocs.u_noise) gl.uniform1f(uniformLocs.u_noise, params.noise)
